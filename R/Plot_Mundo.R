...
Plot.Mundo <- function(
  ...
  if (tipo %NotIN% c("pais", "sinopais")) {
    stop('\u00a1Por favor introduzca un tipo de mapa correcto! Las opciones son: "Pais" y "SiNoPais"', call. = FALSE)
  }
  ...
  if (!all(is.logical(naTo0), is.logical(compacto), is.logical(limpio))) {
    stop("\u00a1Los argumentos 'naTo0', 'compacto' y 'limpio' deben ser un booleano (TRUE o FALSE)!", call. = FALSE)
  }
  if (!missing(colBorde)) {
  ...
  if (!estatico) {
    ...
    if (naTo0) {
      World_Final@data <- World_Final@data |> dplyr::mutate(VarNumeric = replace_na(VarNumeric, 0))
      Comodin <- "%3g"
    } else {
    ...
    TitlePais  <- paste0("<center>", Segregacion, " por<br/>departamento<br/>", Periodo, "</center>")

    # Labels
    Labels_Paises <- sprintf(
      paste0("<strong> %s </strong> <br>", Comodin, " ", tolower(Segregacion)),
      World_Final@data$Country, World_Final@data$Statistic
    ...
    if (tipo == "pais") {
      ...
      Mapa <- Mapa |>
        ...
        addPolygons(
          ...
          highlightOptions = list(
            weight = 4, color = colBorde, dashArray = "", fillOpacity = opacidadClic, bringToFront = TRUE
          ),
          label = Labels_Paises, labelOptions = labelOptions(
            style = list("font-weight" = "normal", padding = "3px 8px"),
            textsize = "12px", direction = "auto"
          )
        ) |>
        addLegend(
          position = "bottomright", pal = pal, values = ~Statistic, opacity = opacidadClic,
          labFormat = labelFormat(prefix = "[", suffix = ")"), title = titulo
        ) |>
        # Adición de los textos indicando el nombre del país.
        addLabelOnlyMarkers(
          lng = Centroides_World$Lon, lat = Centroides_World$Lat,
          label = paste0(sapply(Centroides_World$Country, str_to_title)),
          group = "lupa", labelOptions = labelOptions(
            noHide = TRUE, direction = "top", textOnly = TRUE, textsize = textPaises
          )
        ) |>
      ...
      Mapa <- Mapa |>
        ...
        addPolylines(data = World_Final, stroke = TRUE, color = "white", weight = 2, smoothFactor = 0.05) |>
        addLegend(
        ...
        addLabelOnlyMarkers(
          lng = Centroides_World$Lon, lat = Centroides_World$Lat,
          label = paste0(sapply(Centroides_World$Country, str_to_title)),
          group = "lupa", labelOptions = labelOptions(noHide = TRUE, direction = "top", textOnly = TRUE, textsize = textPaises)
        ) |>
        addSearchFeatures(targetGroups = "lupa", options = searchFeaturesOptions(
    ...
    dots_geom_sf <- list(mapping = aes(fill = {{variable}}), linewidth = LineWidth, color = LineColor)

    if (is.na(estilo$Style)) {
      cortesCute <- waiver()
      if (missingArg(colores)) {
        nLevels <- World_Final_SF |> select({{variable}}) |>
      ...
      if (tolower(estilo$Style) == "intervalo") {
        ...
        NewVar     <- cut(
          World_Final_SF |> select({{variable}}) |> as.data.frame() |> select({{variable}}) |> pull(),
        ...
        cortesCute <- waiver()
        NewVar     <- if_else(
          World_Final_SF |> select({{variable}}) |> as.data.frame() |>
            select({{variable}}) |> pull() > 0,
          "1 o m\u00e1s .", "0 ."
        )
        World_Final_SF <- World_Final_SF |> mutate({{variable}} := NewVar)
      }
    ...
    Mapa <- ggplot(data = World_Final_SF) + do.call(geom_sf, dots_geom_sf) +
      labs(
        title = titulo, subtitle = ParmsLabs$subtitle, x = labelX, y = labelY,
        caption = ParmsLabs$caption, tag = ParmsLabs$tag
      ) +
      coord_sf(crs = "+proj=longlat +datum=WGS84", expand = FALSE, xlim = Xlim, ylim = Ylim) +
    ...
    if (!missingArg(grupo)) { Mapa <- Mapa + facet_wrap(facets = vars({{ grupo }})) }
    if (missingArg(estilo) || is.null(estilo$Labs)) {
      ParmsLabs <- list(subtitle = NULL, caption = NULL, tag = NULL)
    } else { ParmsLabs <- estilo$Labs }
    if (missingArg(estilo) || is.null(estilo$showISO)) {
      ...
      dots_geom_text <- append(list(aes(label = Alfa2), check_overlap = TRUE), showISO)
    ...
    if (!limpio) {
      Mapa <- Mapa +
        ggspatial::annotation_north_arrow(
          location = "bl", which_north = "true", pad_x = unit(0.4, "cm"),
          pad_y = unit(0.4, "cm"), style = ggspatial::north_arrow_fancy_orienteering
        ) +
