...
#'   respectivamente. Si no conoce el dominio del estadístico seleccionado puede
#'   usar el truco de pasar `c(Min, NaN)` como rango.
...
Plot.Radar <- function(datos, categoria, variables,
                       estadistico = c("Promedio", "Mediana", "Varianza", "SD", "CV", "Min", "Max"),
                       colores, rango, ordinal = FALSE, titulo = "",
                       libreria = c("plotly", "echarts"), estilo = NULL) {
  ...
  Groups <- datos %>%
    select({{categoria}}, !!!variables) %>% group_by({{ categoria }}, .drop = FALSE)
