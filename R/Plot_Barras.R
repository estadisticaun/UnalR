...
#'   * `hc.Tema`, `hc.Credits` y `ply.Credits`: Igual uso que en [Plot.Series()]
...
Plot.Barras <- function(datos, categoria, ano, periodo, freqRelativa = FALSE,
                        vertical = TRUE, ordinal = FALSE, colores, titulo = "",
                        labelEje = "N\u00famero de Graduados", addPeriodo = TRUE,
                        textInfo = labelEje, libreria = c("highcharter", "plotly"),
                        estilo = NULL) {
  ...
  if (libreria == "highcharter") {
    ...
    PlotBarras <- highchart() %>%
      hc_add_series(TablaFinal, type = Orientacion, hcaes(x = Clase, y = Y),
    ...
    if (vertical) {
      ...
      PlotBarras <- plot_ly(TablaFinal,
        ...
        layout(
          title = Title, xaxis = list(title = ""), yaxis = list(title = labelEje, ticksuffix = sufijoY),
      ...
      PlotBarras <- plot_ly(TablaFinal,
        ...
        layout(
          title = Title, xaxis = list(title = labelEje, ticksuffix = sufijoY), yaxis = list(title = ""),
          showlegend = ShowLeyenda, autosize = TRUE, margin = Margen
        )
    }
