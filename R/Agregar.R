...
#'
#' @param formula Fórmula en la que el primer componente especificado (antes del
...
#' agrupar (separadas por cualquier operador como `+`, `*`, `:`, etc.).
#' @param frecuencia Vector o lista (dependiendo de la cantidad de variables temporales
#' especificadas) numérica con los periodos que debería tener cada una de éstas.
#' @param datos Un data frame con los microdatos.
#' @param intervalo Vector o lista (dependiendo de la cantidad de variables temporales
...
Agregar <- function(formula, frecuencia, datos, intervalo, textNA = "Sin Informaci\u00f3n", ask = TRUE) {
  ...
  CountNa <- datos %>% select(all_of(Tiempo)) %>% summarise_all(~sum(is.na(.))) %>% sum()
  if (CountNa != 0) {
    stop(paste0("\u00a1La(s) variable(s) '", paste0(Tiempo, collapse = "/"), "' presenta(n) datos faltantes!",
                "\n\t - No tiene sentido continuar con la creaci\u00f3n del agregado si no se tiene la informaci\u00f3n completa de \u00e9sta(s)."), call. = FALSE)
  }
  ...
  UniqueTimes  <- Step1 %>% unite("X", all_of(Tiempo), sep = "-") %>% distinct(X) %>% pull()

  # Creación del agregado.
  Step2 <- Step1 %>%
    group_by(!!!syms(c(Tiempo, Var)), .drop = FALSE) %>%
    summarise("Total" = n()) %>%
    rename("Clase"    = all_of(Var)) %>%
    mutate("Variable" = Var) %>%
    relocate(Variable) %>%
    # ■ Volviendo a la calase original del tiempo en cuestión.
  ...
  if (length(Tiempo) == 1 && is.numeric(frecuencia)) {
    Temp <- expand_grid("Anos" = frecuencia, UniqueFactor) %>% rename(!!Tiempo[1] := Anos, "Clase" = UniqueFactor)
    ...
    stop(paste0("\u00a1Ha ocurrido alguno(s) de los siguientes problemas!:",
                ...
                "\n\t - El n\u00famero de variables temporales y la dimensi\u00f3n de frecuencia no coinciden."), call. = FALSE)
  }
  Step3 <- left_join(Temp, Step2, by = c(Tiempo, "Clase"), keep = FALSE) %>%
    replace_na(list("Total" = 0, "Variable" = Var)) %>%
    relocate(Variable)
  ...
  DB_FactTimes <- Step2 %>% unite("X", all_of(Tiempo), sep = "-") %>% distinct(X) %>% pull()
  DB_JoinTimes <- Step3 %>% unite("X", all_of(Tiempo), sep = "-") %>% distinct(X) %>% pull()
  ...
  if (!missingArg(intervalo)) {
    if (length(Tiempo) == 1 && is.numeric(intervalo)) {
      ...
      Step4 <- Step3
      warning(paste0("\u00a1Ha ocurrido alguno(s) de los siguientes problemas!:",
                     "\n\t - Ha ingresado en la f\u00f3rmula un numero diferente a 1 o 2 variables temporales.",
                     "\n\t - El vector/lista ingresado/a como 'intervalo' contiene valores no num\u00e9ricos.",
                     "\n\u00bb\u00bb No se podr\u00e1 realizar el filtrado por el intervalo de tiempo que ha especificado."), call. = FALSE)
    }
    Agregado <- Step4
  } else {
    if (ask) {
      VarInterface <- menu(c(paste0(last(UniqueTimes),  "\t(\u00faltimo registro de la base de datos)"),
                           ...
                           title = "\u00bCu\u00e1l desea que sea el \u00faltimo periodo por considerar?"
                           )
      CaseFilter <- switch(VarInterface,
                           "1" = last(UniqueTimes),
                           "2" = last(DB_FactTimes),
                           "3" = last(DB_JoinTimes),
                           "4" = NULL
      ...
      if (VarInterface %in% 1:2) {
        Start <- strsplit(first(DB_JoinTimes), "-"); End <- strsplit(CaseFilter, "-")
